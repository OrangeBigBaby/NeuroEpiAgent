...
git-ignored (`manifests/local/`) because it can carry SHA-256s of authorized
files and local data-version metadata. The caller's data root itself is NOT
embedded: the adapter replaces it everywhere with the literal token
`<user-supplied>`.

> **Column counts and schema fingerprints in this document are not
> hand-maintained.** They are produced by `SEERAdapter.inspect`, whose header
> parser (`_parse_header_columns`) routes SEER's quoted, comma-in-value headers
> through the stdlib `csv.reader` rather than a naïve comma split. The earlier
> "269 columns" figure was a comma-split artefact that double-counted headers
> such as `"Race recode (W, B, AI, API)"`; the CSV-aware count is **247**.

## What the local tree contains (anonymized)

| Filename (relative name only) | Byte size | Column count | Schema fingerprint |
| --- | --- | --- | --- |
| `export_C00-C09.csv` | 630,418,267 | 247 | (see inspection JSON) |
| `export_C10-C14.csv` | 131,544,595 | 247 | (see inspection JSON) |
| `export_C15-C26消化器官恶性肿瘤.csv` | 5,289,334,540 | 247 | (see inspection JSON) |
| `export_C30-C39呼吸和胸腔内器官恶性肿瘤.csv` | 3,906,891,646 | 247 | (see inspection JSON) |
| `export_C40-C49.csv` | 3,983,489,441 | 247 | (see inspection JSON) |
| `export_C50 乳腺癌.csv` | 4,060,834,458 | 247 | (see inspection JSON) |
| `export_C51-C53.csv` | 334,534,938 | 247 | (see inspection JSON) |
| `export_C54-C58.csv` | 1,325,517,736 | 247 | (see inspection JSON) |
| `export_C60-C63男性生殖器官恶性肿瘤.csv` | 4,109,931,178 | 247 | (see inspection JSON) |
| `export_C64-C68.csv` | 2,253,791,128 | 247 | (see inspection JSON) |
| `export_C69-C72.csv` | 480,978,014 | 247 | (see inspection JSON) |
| `export_C73-C75.csv` | 766,085,678 | 247 | (see inspection JSON) |
| `export_C76-C80.csv` | 1,473,880,976 | 247 | (see inspection JSON) |

> The byte sizes above are an inventory; the inspection JSON is the
> authoritative copy. The 13 files all share the same **247-column** schema.
> Two distinct kinds of evidence support that statement and must not be
> conflated: (a) the **synthetic** 13-file test in
> `tests/test_seer_adapter.py::TestCrossFileSchema` verifies that the adapter
> *code* detects schema consistency/divergence; (b) a **real-directory**
> metadata-only smoke check (run locally, output written to the git-ignored
> `manifests/local/`) confirmed the local 13 files produce a single schema
> fingerprint. The synthetic test does not exercise real data, and the
> real-directory smoke is not an automated test.

## Schema (the 247 column names)
...
> The remaining columns follow the same pattern (recodes, derived
...
> SEER*Stat wrote it. The count (247) is the length of that CSV-parsed
> list, regenerated by the adapter; it is not a hand-maintained constant.
